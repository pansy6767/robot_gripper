...
int main() {
    ...
    for (int i = 0; devices[i]; i++) {
        ...
        // 打印设备信息
        std::cerr << "U2CANFD_DEV " << i << ":" << std::endl;
        std::cerr << "  VID: 0x" << std::hex << desc.idVendor << std::endl;
        std::cerr << "  PID: 0x" << std::hex << desc.idProduct << std::endl;
        std::cerr << "  SN: " << (serial_number[0] ? serial_number : "[No serial number]") << std::endl;
        std::cerr << std::endl;
