...
def movejp_type(pose, speed=0.5, lock_last_joint=True):
    """
    末端位姿运动
    ...
    Args:
        ...
        speed: 速度比例
        lock_last_joint: 是否锁定最后一个关节（夹爪关节）
    """
    global arm
    
    # 获取当前关节值，用于锁定最后关节
    current_joints = arm.get_current_joint_values()
    last_joint_value = current_joints[-1]  # 记录最后一个关节的值
    
    if lock_last_joint:
        rospy.loginfo("锁定最后关节，当前值: %.4f rad" % last_joint_value)
    ...
    # 使用位置目标而不是完整位姿目标，让求解器更容易找到解
    # 这样可以放宽姿态约束，提高规划成功率
    target_position = [pose[0], pose[1], pose[2]]
    arm.set_position_target(target_position, end_effector_link)
    ...
    if success and lock_last_joint:
        # 修改轨迹，锁定最后一个关节
        rospy.loginfo("修改轨迹，锁定最后关节为 %.4f rad..." % last_joint_value)
        
        for point in trajectory.joint_trajectory.points:
            # 转换为列表以便修改
            positions = list(point.positions)
            positions[-1] = last_joint_value  # 锁定最后关节
            ...
            if point.velocities:
                ...
                velocities[-1] = 0.0
                point.velocities = tuple(velocities)
            
            # 将最后关节的加速度设为0
            if point.accelerations:
                accelerations = list(point.accelerations)
                accelerations[-1] = 0.0
                point.accelerations = tuple(accelerations)
...
def movejp_type_full_pose(pose, speed=0.5, lock_last_joint=True):
    """
    末端位姿运动（完整位姿约束版本）
    如果 movejp_type 不满足需求，可以用这个函数
    
    Args:
        pose: [x, y, z, qx, qy, qz, qw] 目标位姿
        speed: 速度比例
        lock_last_joint: 是否锁定最后一个关节
    """
    global arm
    
    # 获取当前关节值
    current_joints = arm.get_current_joint_values()
    last_joint_value = current_joints[-1]
    ...
    # 如果完整位姿规划失败，尝试只用位置目标
    if not success:
        rospy.logwarn("完整位姿规划失败，尝试只用位置目标...")
        arm.clear_pose_targets()
        arm.set_position_target([pose[0], pose[1], pose[2]], end_effector_link)
        plan = arm.plan()
...
def catch(result, current_pose, current_joints, is_defective=False):
    ...
    # 第1步：移动到物体上方7cm（锁定最后关节）
    rospy.loginfo("Step 1: 移动到物体上方 7cm")
    success = movejp_type([result[0], result[1], result[2] + 0.07,
                           current_pose.orientation.x, current_pose.orientation.y,
                           current_pose.orientation.z, current_pose.orientation.w], 
                          speed=0.3, lock_last_joint=True)
    if not success:
        rospy.logerr("Step 1 失败!")
        return
    ...
    # 第4步：抬起（锁定最后关节）
    rospy.loginfo("Step 4: 抬起物体")
    success = movejp_type([result[0], result[1], result[2] + 0.10,
                           current_pose.orientation.x, current_pose.orientation.y,
                           current_pose.orientation.z, current_pose.orientation.w], 
                          speed=0.3, lock_last_joint=True)
    if not success:
...
if __name__ == '__main__':
    ...
    rospy.loginfo("【修改】运动时最后一个关节保持不动")
    rospy.loginfo("=" * 60)
    rospy.loginfo("使用方法:")
    rospy.loginfo("  基本抓取:")
    rospy.loginfo("    rostopic pub /choice_object std_msgs/String \"red_circle\"")
